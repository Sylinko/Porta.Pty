...
internal class PtyProvider : Unix.PtyProvider
{
    ...
    public override Task<IPtyConnection> StartTerminalAsync(PtyOptions options, TraceSource trace, CancellationToken cancellationToken)
    {
        ...
        var term = new PtyTermios(
            TermInputFlag.ICRNL | TermInputFlag.IXON | TermInputFlag.IXANY | TermInputFlag.IMAXBEL | TermInputFlag.BRKINT | TermInputFlag.IUTF8,
            TermOutputFlag.NONE, // Disable all output processing for raw terminal emulation
            TermControlFlag.CREAD | TermControlFlag.CS8 | TermControlFlag.HUPCL,
            TermLocalFlag.ICANON | TermLocalFlag.ISIG | TermLocalFlag.IEXTEN | TermLocalFlag.ECHO | TermLocalFlag.ECHOE | TermLocalFlag.ECHOK |
