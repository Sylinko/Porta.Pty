...
internal static class NativeMethods
{
    ...
    internal extern static PtySpawnResult pty_spawn(
    ...
    /// </summary>
    [DllImport(LibPortaPty, SetLastError = true)]
    internal extern static int pty_resize(int masterFd, ushort rows, ushort cols);

    /// <summary>
    ///     Sends a signal to the child process.
    /// </summary>
    [DllImport(LibPortaPty, SetLastError = true)]
    internal extern static int pty_kill(int pid, int signal);

    /// <summary>
    ///     Waits for the child process to exit.
    /// </summary>
    [DllImport(LibPortaPty, SetLastError = true)]
    internal extern static int pty_waitpid(int pid, ref int status, int options);

    /// <summary>
    ///     Closes the PTY master file descriptor.
    /// </summary>
    [DllImport(LibPortaPty, SetLastError = true)]
    internal extern static int pty_close(int masterFd);

    /// <summary>
    ///     Gets the last error code from the native library.
    /// </summary>
    [DllImport(LibPortaPty)]
    internal extern static int pty_get_errno();
